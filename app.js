...
const aws = require("aws-sdk"); // TODO: upgrade to v3 before September 8, 2025
...
const logger = require("pino-http")({
  ...
  autoLogging: {
    ignore: (req) => {
      return ["/healthz", "/readyz", "/metrics"].includes(req.path);
    },
  },
  genReqId: () => randomUUID(),
  serializers: {
...
// TODO: Need to add these permissions to the service role & not use the keys here
aws.config.update({
...
// TODO: Production should not use { origin: "*" }, but also, we should maybe
// not be using cors at all
...
async function GCMpush(certificate, device, payload) {
  const client = http2.connect(
    "https://fcm.googleapis.com:443/v1/projects/api-6882780734960683553-445906"
  );
...
async function SESpush(email, payload) {
  return SES.sendEmail({
    Source: AWS_SES_FROM,
    ReplyToAddresses:
      typeof payload.from === "string" ? payload.from.split(",") : [],
    Destination: {
      ToAddresses: [email],
...
async function SLACKpush(hook, message) {
  ...
  const buffer = Buffer.from(
    JSON.stringify({ text: message.content || "<no message body>" })
  );
  const request = client.request({
    [":method"]: "POST",
    [":path"]: `/services/${hook}`,
...
// TODO: consider moving api key validation to middleware
...
app.post("/push", express.json(), async (req, res) => {
  ...
  let verify2 = ["apns", "gcm", "mailto", "sms", "slack"].includes(
    req.body["push_type"]
  );
  let verify2a =
    verify1 &&
    ["apns", "gcm", "mailto", "sms", "slack"].includes(
      req.body["device_token"].split(/:(.+)/, 2)[0]
    );
  let verify3 = typeof req.body["payload"] === "object";
  if (!verify0)
    return res
      .status(400)
      .json({ error: "bad request: valid api_key is required" });
  else if (!verify1)
    return res
      .status(400)
      .json({ error: "bad request: device_token must be a string" });
  else if (!verify2 && !verify2a)
    return res.status(400).json({
      error:
        "bad request: push_type must be one of ['apns', 'gcm', 'mailto', 'sms', 'slack']",
    });
  else if (!verify3)
    return res
      .status(400)
      .json({ error: "bad request: payload must be an object" });
  if (verify2a) {
...
// TODO: If nobody is using '/' anymore, let's remove the comment above
app.put(["/log", "/"], express.text({ type: "*/*" }), async (req, res) => {
  // Some types of logging messages are not allowed (PHI, etc.)
  if (
    typeof req.body !== "string" ||
    req.body.includes("Protected health data is inaccessible")
  )
  ...
  if (req.query.stream === "slack") {
    ...
    console.log(
      `[${req.query.level || "info"}] [${req.query.origin || "unknown"}] ${(
        req.body || ""
      ).trim()}`
    );
    return res.status(200).json({});
...
app.get("/readyz", (req, res) => {
  req.log.info("heya!");
  console.log("ready point called");
...
app.get("/version", (_, res) => {
  return res.json({
    docker: {
      date: process.env.DOCKER_BUILD_DATE,
      sha256sum: process.env.DOCKER_SHA256,
    },
    git: {
...
// TODO: consider splitting this code into proper node 'bin'
async function main() {
  if (arguments.length < 3 || !["apns", "gcm"].includes(arguments[0])) {
    console.log(`Usage: ./push.js <apns|gcm> <DEVICE_TOKEN> <PAYLOAD>`);
    console.log(
      `This utility allows you to push a JSON payload to a device with the mindLAMP app (either iOS or Android) installed.`
    );
    process.exit(-1);
  } else {
    try {
